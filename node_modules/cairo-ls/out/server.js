...
function validateTextDocument(textDocument) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (settings.disableHintValidation) {
            const disabled = glob.sync(settings.disableHintValidation).map((v) => { return "file://" + path.resolve(v); });
            disableHintValidation = disabled.some((v) => v == textDocument.uri);
        }
        yield exec(commandPrefix + "cd " + tempFolder + " && " + getCompileCommand(settings, tempFiles, textDocumentContents, disableHintValidation), (error, stdout, stderr) => {
            // delete temp files
 ...
 */
function getCompileCommand(settings, tempFiles, textDocumentContents, disableHintValidation = false) {
 ...
 */
function getCairoPaths(sourceDir, cairoPath) {
    const paths = [];
 ...
 */
function getCairoPathParam(paths) {
    let result = '';
    if (paths.length > 0) {
        result += '--cairo_path="';
